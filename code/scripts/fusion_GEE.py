...
#ee.Authenticate()
ee.Initialize(project='theta-arcana-484116-h7') #MODIFY ACCORDING TO YOUR PROJECT NAME
...
def main():
    ...
    ==============================
    """
    cfg = json.load(open(r"/workspaces/codespaces-blank/scripts/Parameters.json", 'r'))
    AOI_PATH = r"/workspaces/codespaces-blank/AOI/grid_372.shp"     #MODIFY PATH ACCORDING TO YOUR SHAPEFILE LOCATION
    ...
    ====================================
    """
    parent_folder = "projects/theta-arcana-484116-h7/assets/folder"  #MODIFY ACCORDING TO YOUR EARTH ENGINE ASSET PATH
    ...
    if EXPORT_PCA:
        ...
        the next heavy computation to avoid exceeding GEE's computation power"""
        # Safely obtain band names (use getInfo guarded by try/except to avoid
        # passing null to Image.select). If band names can't be retrieved,
        # skip renaming to avoid null parameters.
        original_band_names = None
        try:
            original_band_names = NDVI_smoothed.bandNames().getInfo()
        except Exception:
            original_band_names = None

        # Build an export image: if S2 and band names are available, create
        # renamed bands for export; otherwise export the image as-is.
        if OPTICAL_MISSION == 'S2' and original_band_names:
            S2_ids = ee.List(original_band_names)
            def cat_S2_id(element):
                return ee.String('S2_').cat(element)
            S2_ids_new = S2_ids.map(cat_S2_id)
            NDVI_for_export = NDVI_smoothed.select(S2_ids, S2_ids_new)
        else:
            NDVI_for_export = NDVI_smoothed

        subassets = ee.data.listAssets({'parent': subasset_folder})['assets']
        subasset_names = [asset['name'].split('/')[-1] for asset in subassets]
        smoothed_name = 'smoothedImage202' + nameSuffix
        ...
        if smoothed_name not in subasset_names:
            task = utilities.export_image_toasset(
                ...
                AOI,
                smoothed_id,
                description='Smoothed NDVI 202')
            check_task_status(task)

        # Load the stored smoothed image and, if necessary, map its band
        # names back to the originals. Use getInfo guarded by try/except to
        # avoid passing nulls to select.
        NDVI_smoothed = ee.Image(smoothed_id)
        if OPTICAL_MISSION == 'S2' and original_band_names:
            try:
                loaded_band_names = NDVI_smoothed.bandNames().getInfo()
            except Exception:
                loaded_band_names = None
            if loaded_band_names:
                NDVI_smoothed = NDVI_smoothed.select(
                    ee.List(loaded_band_names), ee.List(original_band_names))
